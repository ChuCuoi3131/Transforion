<?php
    ...
    if (is_null(filter_input(INPUT_GET, 'nv_id'))){
        $nv_id = 1;
    }else{
        $nv_id = filter_input(INPUT_GET, 'nv_id');
    }

    $nv = get_nv_by_id($nv_id);
    ...
    <main>
        ...
        <section class="mb-3">
            <div class="container py-5">
                <h2 class="text-center"><i class="fa-solid fa-list" style="font-size: 40px;"></i> NHÂN VẬT </h2>
                <div class="row row-cols-1 row-cols-md-3 g-4 py-3">
                </div>
            </div>
            <div class="container">
            <?php foreach ($nv as $n): ?>
                <img class="img-fluid main-char-img " src="<?php echo $n['AnhNv']; ?>" alt="">
                <h4 class="">
                    <?php echo $n['MotaNv']; ?>
                </h4>
            <?php endforeach; ?>
